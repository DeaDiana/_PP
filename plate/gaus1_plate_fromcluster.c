...
void generatePartOfMatrix(double** partOfMatr, int *loc_hghts, int *starts_v, int rank, int M_size, double hx_step, double hy_step, int Nx_number, int Ny_number)
{
    double a_coef = -2 * (1 / (hx_step * hx_step) + 1 / (hy_step * hy_step));
    double b_coef = 1 / (hx_step * hx_step);
    double c_coef = 1 / (hy_step * hy_step);
    ...
    for(int i = 0; i < M_size; i++)
    {
        if(((i >= starts_v[rank]) && (i < (starts_v[rank] + loc_hghts[rank])))
        {
            partOfMatr[i - starts_v[rank]][i] = a_coef;
            if((i + 1) < M_size)
            {
                partOfMatr[i - starts_v[rank]][i + 1] = b_coef;
                partOfMatr[i + 1 - starts_v[rank]][i] = b_coef;
            }
            if((i + Nx_number) < M_size)
            {
                partOfMatr[i - starts_v[rank]][i + Nx_number] = c_coef;
                partOfMatr[i + Nx_number - starts_v[rank]][i] = c_coef;
            }
        }
        if(i < (starts_v[rank] + loc_hghts[rank]))
        {
            break;
        }
    }
...
{
    int k = point_y * Nx_number + point_x;
    matr[k][k] = BIG_NUMBER;
...
void firstKindBoundaryConditionForPart(double** partOfMatr, int *loc_hghts, int *starts_v, int rank, int M_size, int Nx_number, int point_x, int point_y)
{
    int k = point_y * Nx_number + point_x;
    matr[k][k] = BIG_NUMBER;
    matr[k][M_size] = 1.0 * BIG_NUMBER;
    ...
    {
        k = 0 * Nx_number + j_node;
        if((k >= starts_v[rank]) && (k < (starts_v[rank] + loc_hghts[rank])))
        {
            partOfMatr[k - starts_v[rank]][k] = BIG_NUMBER;
            partOfMatr[k - starts_v[rank]][M_size] = 0.5 * BIG_NUMBER;
        }
        if(k < (starts_v[rank] + loc_hghts[rank]))
        {
            break;
        }
    }
...
//---------------------------------------parallel computation
    ...
    generatePartOfMatrix(partOfMatr, loc_hghts, starts_v, rank, M_size, hx_step, hy_step, Nx_number, Ny_number);
    firstKindBoundaryConditionForPart(partOfMatr, loc_hghts, starts_v, rank, M_size, Nx_number, Nx_number / 2, Ny_number / 2);
...
//gauss algorithm
    ...
    if(NULL == (file = fopen("big_plate", "wb"))) {
       fprintf(stderr, "file with source matrix wasn't opened\n");
        exit(1); 
    }
